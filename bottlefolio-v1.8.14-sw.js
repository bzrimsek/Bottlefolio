...
const CACHE_NAME = 'bottlefolio-v1.8.14';
...
self.addEventListener('install', e => {
  e.waitUntil(
    caches.open(CACHE_NAME).then(c =>
      c.addAll(SHELL).then(() =>
        // Best effort, and never a reason to fail: a stale data.json is
        // refetched on first use.
        Promise.all(DATA.map(d => c.add(d).catch(() => null)))
      )
    )
  );
});
