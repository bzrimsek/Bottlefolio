...
const CACHE_NAME = 'bottlefolio-v2.4.17';
...
/* The maskable icons are here too, or the manifest points at files the
   service worker never fetched - which shows as a blank home-screen icon
   on a device that installed the app offline. */
const EXTRAS = ['./manifest.json', './mark.png', './icon-192.png',
                './icon-512.png', './icon-mask-192.png',
...
self.addEventListener('install', e => {
  e.waitUntil(
    caches.open(CACHE_NAME).then(c =>
      /* FROM THE NETWORK. addAll may be answered from the browser's own
         HTTP cache, so a new worker could install the build before it. */
...
self.addEventListener('fetch', e => {
  ...
  if (isData(url)) {
    ...
    /* BUT NOT FOREVER. One bar of signal in a shop stalled this fetch for as
       long as the network cared to take, and Home waited on it: 5.1 seconds
       of blank screen, measured, against 0.1 with no network at all
       (review, 2026-09-15). A slow network was worse than none. So the
       network gets a second and a half; after that the saved copy answers,
       and the network's reply, when it lands, still refreshes the cache for
       next time. With no saved copy it keeps waiting — a late shelf beats
       no shelf. */
    e.respondWith(new Promise(resolve => {
      let answered = false;
      const answer = res => {
        if (!answered && res) { answered = true; resolve(res); }
      };
      ...
      const slow = setTimeout(() => {
        caches.match(e.request).then(hit => answer(hit));
      }, 1500);
      net.then(res => { clearTimeout(slow); answer(res); })
        .catch(() => {
          clearTimeout(slow);
          caches.match(e.request).then(hit => answer(hit || Response.error()));
        });
    }));
    return;
  }
